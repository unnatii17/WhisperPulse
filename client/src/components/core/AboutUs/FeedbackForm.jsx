...
import { FaComment, FaPaperPlane, FaStar, FaHeart, FaCheckCircle } from 'react-icons/fa';
...
const FeedbackForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(sendFeedback(token, { message: feedback, rating }));
    setSubmitted(true);
    setTimeout(() => {
      setSubmitted(false);
      setFeedback("");
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        <div className="inline-flex items-center justify-center p-3 bg-purple-500/10 rounded-2xl mb-3">
      ...
      {submitted ? (
        ...
        >
          ...
          <h4 className="font-bold text-gray-900 dark:text-white">Thank You for Your Feedback! ❤️</h4>
          <p className="text-xs text-gray-600 dark:text-gray-300 mt-1">Your thoughts mean the world to us.</p>
        </motion.div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-center gap-2 mb-2">
            {[1, 2, 3, 4, 5].map((star) => (
              ...
              >
                <FaStar className={star <= rating ? "text-amber-400" : "text-gray-300 dark:text-gray-600"} />
              </button>
            ))}
          </div>
