...
const OTP = () => {
    ...
    const handleOnSubmit=(e)=>{
      ...
      // Reconstruct the File from sessionStorage base64 (avoids storing File in Redux)
      let avatarFile = null;
      const avatarBase64 = sessionStorage.getItem("avatarBase64");
      const avatarName = sessionStorage.getItem("avatarName");
      const avatarType = sessionStorage.getItem("avatarType");
      if (avatarBase64 && avatarName && avatarType) {
        const byteString = atob(avatarBase64.split(",")[1]);
        const ab = new ArrayBuffer(byteString.length);
        const ia = new Uint8Array(ab);
        for (let i = 0; i < byteString.length; i++) ia[i] = byteString.charCodeAt(i);
        avatarFile = new File([ab], avatarName, { type: avatarType });
